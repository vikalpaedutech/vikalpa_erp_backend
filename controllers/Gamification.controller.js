...
//Create gamification contestant api
export const CreateGamificationContestant = async (req, res) => {

  const {unqUserObjectId, schoolId, batch} = req.body;
  console.log(req.body)

  try {
    
    const response = await GamificationContestant.create(req.body)
...
export const selfAttendancePoint = async (req, res) => {
  ...
  const { unqObjectId, batch, unqIdOfPointObject, date, schoolId } = req.body;
...
export const studentAttendance = async (req, res) => {
  ...
  // ──────────────────────────────────────────────────────────────
  // ✅ VALIDATE IF USER IS A VALID CONTESTANT FOR THIS SCHOOL & BATCH
  // ──────────────────────────────────────────────────────────────
  
  try {
    // First, check if the user exists in GamificationContestant collection
    const contestant = await GamificationContestant.findOne({
      unqUserObjectId: unqObjectId
    });

    if (!contestant) {
      console.log("❌ User not found in GamificationContestant collection");
      return res.status(403).json({
        status: "Failed",
        message: "User is not a registered contestant",
        data: null
      });
    }

    // Check if the schoolId from request exists in contestant's schoolId array
    const isValidSchool = contestant.schoolId.includes(schoolId);
    
    // Check if the batch from request exists in contestant's batch array
    const isValidBatch = contestant.batch.includes(batch);

    if (!isValidSchool || !isValidBatch) {
      console.log(`❌ Invalid school or batch for this contestant. School: ${schoolId}, Batch: ${batch}`);
      console.log(`✅ Allowed schools: ${contestant.schoolId.join(', ')}`);
      console.log(`✅ Allowed batches: ${contestant.batch.join(', ')}`);
      
      return res.status(403).json({
        status: "Failed",
        message: "Invalid school or batch for this contestant",
        data: {
          allowedSchools: contestant.schoolId,
          allowedBatches: contestant.batch,
    ...
    console.log("✅ Contestant validation passed!");
    console.log(`✅ School: ${schoolId} is valid for this user`);
    console.log(`✅ Batch: ${batch} is valid for this user`);

  } catch (error) {
    console.log("Error in contestant validation:", error);
    return res.status(500).json({
      status: "Failed",
      message: "Error validating contestant",
      error: error.message
    });
  }

  // ──────────────────────────────────────────────────────────────
  // ✅ PROCEED WITH POINT LOGIC
  // ──────────────────────────────────────────────────────────────
...
export const callingAbsentee = async (req, res) => {
  console.log("I am in Gamification.controller.js, api: callingAbsentee");

  const { batch, schoolId, unqObjectId, unqIdOfPointObject, date } = req.body;
  console.log("Calling Absentee Payload:", req.body);
...
export const disciplinary = async (req, res) => {
  ...
  console.log("Disciplinary Payload:", payload);
  ...
  try {
    ...
    // Convert schoolId to array for validation
    let schoolIdsToValidate = [];
    if (Array.isArray(schoolId)) {
      schoolIdsToValidate = schoolId;
    } else if (typeof schoolId === 'string') {
      schoolIdsToValidate = schoolId.split(',').filter(id => id.trim());
    }

    // Convert batch to array for validation
    let batchesToValidate = [];
    if (Array.isArray(batch)) {
      batchesToValidate = batch;
    } else if (typeof batch === 'string') {
      batchesToValidate = [batch];
    }

    // Check if ALL requested schools are valid for this contestant
    const invalidSchools = schoolIdsToValidate.filter(sid => !contestant.schoolId.includes(sid));
    const invalidBatches = batchesToValidate.filter(b => !contestant.batch.includes(b));

    if (invalidSchools.length > 0 || invalidBatches.length > 0) {
      console.log(`❌ Invalid schools or batches for this contestant.`);
      console.log(`❌ Invalid Schools: ${invalidSchools.join(', ')}`);
      console.log(`❌ Invalid Batches: ${invalidBatches.join(', ')}`);
      console.log(`✅ Allowed schools: ${contestant.schoolId.join(', ')}`);
      console.log(`✅ Allowed batches: ${contestant.batch.join(', ')}`);
      ...
      const errorResponse = {
        ...
        data: {
          allowedSchools: contestant.schoolId,
          allowedBatches: contestant.batch,
          requestedSchools: schoolIdsToValidate,
          requestedBatches: batchesToValidate,
    ...
    console.log("✅ Contestant validation passed!");
    console.log(`✅ All requested schools are valid for this user`);
    console.log(`✅ All requested batches are valid for this user`);
  ...
  if (!GamificationPointLogicForStudentAttendance || GamificationPointLogicForStudentAttendance.length === 0) {
    console.log("⚠️ No disciplinary gamification logic found");
    const noDataResult = {
      status: "Skipped",
      message: "No disciplinary gamification logic found",
      pointValue: 0
    };
  ...
  try {
    ...
    console.log("Error in disciplinary:", error);
...
export const marks = async (req, res) => {
  console.log("I am in Gamification.controller.js, api: marks");
  ...
  console.log("Marks payload:", req.body);
  ...
  if (!GamificationPointLogicForMarks || GamificationPointLogicForMarks.length === 0) {
    console.log("⚠️ No marks gamification logic found");
    const noDataResult = {
      status: "Skipped",
      message: "No marks gamification logic found",
      pointValue: 0
    };
